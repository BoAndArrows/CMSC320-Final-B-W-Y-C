...
class SineWithPolyModel:
    ...
    def __init__(self, degree = 1, iterations = 1000, lr = 0.01):
        ...
        self.__regressor = __Regression(iterations = iterations, lr = lr)
...
class __Regression:
    ...
    def __init__(self, iterations = 1000, lr = 0.01):
        ...
        self.sin_amp = 1.0       # Amplitude of the sine
        self.sin_ang_freq = 1.0  # Angular frequency of the sine
    ...
    def fit(self, X, Y):
        ...
        self.poly_weights = np.random.randn(X.shape[1])
        ...
        for iter in range(self.iterations):
            ...
            diff = np.array([a - b for a,b in zip(Y,Y_pred)]) # I don't trust numpy
            ...
            sin_term = np.sin(self.sin_ang_freq * X)
            cos_term = np.cos(self.sin_ang_freq * X)

            sin_amp_grad = 2 * np.mean([a*b for a,b in zip(sin_term, diff)])
            sin_ang_freq_grad = (2*self.sin_amp) * np.mean([a*b*c for a,b,c in zip(diff, cos_term, X)])
    ...
    def predict(self, X):
        ...
        return np.dot(X, self.poly_weights) + self.bias + \
                self.sin_amp * np.sin(self.sin_ang_freq * X)